...
type ThoughtType = 'insight' | 'question' | 'evidence' | 'conclusion';
type ThoughtStatus = 'active' | 'revised' | 'deprecated';

interface ThoughtStep extends Record<string, unknown> {
  thought: string;
...
function isValidThoughtStep(args: Record<string, unknown> | undefined): args is ThoughtStep {
  ...
  const hasValidOptionalFields =
    ...
    (args.type === undefined || ['insight', 'question', 'evidence', 'conclusion'].includes(args.type as string)) &&
    (args.status === undefined || ['active', 'revised', 'deprecated'].includes(args.status as string)) &&
...
class ThoughtGraph {
  ...
  getRelatedThoughts(thoughtNumber: number): ThoughtStep[] {
    const related = this.edges.get(thoughtNumber) || new Set();
    return Array.from(related).map(num => 
      this.thoughts.find(t => t.thoughtNumber === num)
    ).filter((t): t is ThoughtStep => t !== undefined);
  }
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      switch (request.params.name) {
        case 'add_thought': {
          if (!isValidThoughtStep(request.params.arguments)) {
            throw new McpError(
              ...
              'Invalid thought step parameters'
            );
          }
        ...
        case 'get_thoughts': {
          const format = (request.params.arguments as { format?: string })?.format || 'json';
          if (format === 'markdown') {
            return {
              content: [
                {
        ...
        case 'get_related_thoughts': {
          const { thoughtNumber } = request.params.arguments as { thoughtNumber: number };
          return {
            content: [
              {
                type: 'text',
